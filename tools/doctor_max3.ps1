...
param(
  ...
  [switch]$Install,
  [switch]$AutoFix,
  [switch]$DeepNpmAudit,   # optional: runs npm ls --depth=0 and npm config get prefix/cache
  [switch]$OpenReport,
  [switch]$Pause
)
...
# Avoid alias collisions (r/R is Invoke-History in Windows PowerShell)
Remove-Item Alias:r -ErrorAction SilentlyContinue | Out-Null
...
function Add-RepLine([string]$s) { [void]$script:Report.AppendLine($s) }

function Assert-Path([string]$p, [string]$label) {
  if (-not (Test-Path -LiteralPath $p)) { Add-RepLine ("- [MISSING] {0}: {1}" -f $label, $p); return $false }
  Add-RepLine ("- [OK] {0}: {1}" -f $label, $p); return $true
}

function Resolve-External([string]$name) {
  try {
...
function Get-CommandVersion([string]$cmd, [string]$args) {
  try {
    $resolved = Resolve-External $cmd
    if (-not $resolved) { return @{ ok=$false; exit=$null; text="Not found in PATH: $cmd" } }
    ...
    # Run via cmd.exe to support .cmd reliably
    $cmdLine = '"' + $resolved + '" ' + $args
    $p = Start-Process -FilePath "cmd.exe" -ArgumentList @("/d","/s","/c",$cmdLine) -NoNewWindow -Wait -PassThru -RedirectStandardOutput $tmpOut -RedirectStandardError $tmpErr
    $out = ""
    if (Test-Path $tmpOut) { $out = (Get-Content $tmpOut -Raw).Trim() }
    if (-not $out -and (Test-Path $tmpErr)) { $out = (Get-Content $tmpErr -Raw).Trim() }
    return @{ ok=$true; exit=$p.ExitCode; text=$out; path=$resolved }
  } catch {
...
function Invoke-LoggedCmd {
  ...
  Add-RepLine ""
  Add-RepLine ("## {0}" -f $Title)

  $resolved = Resolve-External $FilePath
  if (-not $resolved) {
    Add-RepLine ("- [MISSING] Command not found in PATH: {0}" -f $FilePath)
    Add-RepLine ("- ExitCode: **127**")
    return 127
  }

  $argStr = ""
  if ($ArgumentList -and $ArgumentList.Count -gt 0) {
    $argStr = ($ArgumentList | ForEach-Object { Quote-CmdArg $_ }) -join " "
  }
  $cmdLine = '"' + $resolved + '"' + ($(if ($argStr) { " " + $argStr } else { "" }))

  Add-RepLine ("- Command: {0}" -f $cmdLine)
  Add-RepLine ("- WorkingDirectory: {0}" -f $WorkDir)

  $sw = [System.Diagnostics.Stopwatch]::StartNew()
  $p = Start-Process -FilePath "cmd.exe" -ArgumentList @("/d","/s","/c",$cmdLine) -WorkingDirectory $WorkDir -NoNewWindow -Wait -PassThru -RedirectStandardOutput $StdOutPath -RedirectStandardError $StdErrPath
  $sw.Stop()

  Add-RepLine ("- ExitCode: **{0}**" -f $p.ExitCode)
  Add-RepLine ("- Duration: **{0:n2}s**" -f $sw.Elapsed.TotalSeconds)
  Add-RepLine ("- Stdout: {0}" -f $StdOutPath)
  Add-RepLine ("- Stderr: {0}" -f $StdErrPath)
...
function Parse-ErrorLocationsFromText([string]$text) {
  ...
  foreach ($m in $rxA.Matches($text)) {
    $locs.Add([pscustomobject]@{ File = Normalize-Path $m.Groups[1].Value; Line = [int]$m.Groups[3].Value; Col = [int]$m.Groups[4].Value; Raw = $m.Value.Trim() }) | Out-Null
  }
...
function Write-CodeFrameToReport([string]$file, [int]$line, [int]$col, [int]$context) {
  Add-RepLine ("### Code Frame: {0}:{1}:{2}" -f $file, $line, $col)
  Add-RepLine ""

  if (-not (Test-Path -LiteralPath $file)) { Add-RepLine "> File not found on disk."; return }
  ...
  for ($i=$from; $i -le $to; $i++) {
    ...
    $textLine = $lines[$i-1]
    if ($i -eq $line) {
      Add-RepLine ($prefix + $textLine)
      $caretPad = " " * ([Math]::Max(0, ($prefix.Length + $col - 1)))
      Add-RepLine ($caretPad + "^")
    } else {
      Add-RepLine ($prefix + $textLine)
    }
  }
...
function Focus-Checks([string]$focusAbs) {
  Add-RepLine ""
  Add-RepLine "## Focus File Checks"

  if (-not (Test-Path -LiteralPath $focusAbs)) { Add-RepLine ("- [MISSING] Focus file: {0}" -f $focusAbs); return }
  ...
  $beginIdx = Find-Marker $L 'UI_SHELL_THUMB_STRIP_RENDER_BEGIN'
  if ($beginIdx -ge 0) {
    if (Is-InsideStyleObject $L $beginIdx) { Add-RepLine "- [FAIL] Thumb strip marker is inside style={{}}." }
    else { Add-RepLine "- [OK] Thumb strip marker not inside style={{}}." }
  } else {
    Add-RepLine "- [INFO] UI_SHELL_THUMB_STRIP_RENDER_BEGIN not found."
  }
}
...
Add-RepLine ("# Doctor Max Report - {0}" -f $stamp)
Add-RepLine ""
Add-RepLine "## Preflight"
Add-RepLine ("- RepoRoot: {0}" -f $RepoRoot)
Add-RepLine ("- PowerShell: {0}" -f $PSVersionTable.PSVersion)
Add-RepLine ("- OS: {0}" -f ([System.Environment]::OSVersion.VersionString))
Add-RepLine ("- User: {0}" -f $env:USERNAME)
Add-RepLine ""

$okRepo = $true
if (-not (Test-Path -LiteralPath $RepoRoot -PathType Container)) { $okRepo = $false; Add-RepLine "- [MISSING] RepoRoot folder not found." }
...
Add-RepLine ""
Add-RepLine "## Tooling Versions"
$node = Get-CommandVersion "node" "-v"
$npm  = Get-CommandVersion "npm"  "-v"
$npx  = Get-CommandVersion "npx"  "-v"
$git  = Get-CommandVersion "git"  "--version"

Add-RepLine ("- Node: {0}" -f ($(if ($node.ok) { $node.text + " (" + $node.path + ")" } else { "[MISSING] " + $node.text })))
Add-RepLine ("- npm: {0}"  -f ($(if ($npm.ok)  { $npm.text  + " (" + $npm.path  + ")" } else { "[MISSING] " + $npm.text })))
Add-RepLine ("- npx: {0}"  -f ($(if ($npx.ok)  { $npx.text  + " (" + $npx.path  + ")" } else { "[MISSING] " + $npx.text })))
Add-RepLine ("- git: {0}"  -f ($(if ($git.ok)  { $git.text  + " (" + $git.path  + ")" } else { "[MISSING] " + $git.text })))

if (-not $okRepo) {
  Add-RepLine ""
  Add-RepLine "## STOP"
  Add-RepLine "Repo preflight failed."
...
if ($Clean) {
  ...
  Add-RepLine ("- Removed: {0}" -f ($(if ($removed.Count -gt 0) { $removed -join ", " } else { "(none)" })))
}

$pm = Choose-PackageManager $RepoRoot
Add-RepLine ""
Add-RepLine "## Package Manager"
Add-RepLine ("- Detected: {0}" -f $pm.name)
...
if ($Install) {
  ...
  $cmd = $pm.install
  if ($pm.name -eq "npm" -and $pm.ci -ne $null -and (Test-Path (Join-Path $RepoRoot "package-lock.json"))) { $cmd = $pm.ci; Add-RepLine "- Using npm ci." }
...
if ($AutoFix) {
  ...
  if (Test-Path -LiteralPath $focusAbs) {
    ...
    Add-RepLine ("- Backup created: {0}" -f $bak)
    ...
    if ($mv.moved) { Add-RepLine ("- Fixed: {0}" -f $mv.reason); $changed = $true }
    else { Add-RepLine ("- Note: {0}" -f $mv.reason) }

    if ($changed) {
      Write-Utf8NoBom $focusAbs ($L -join "`r`n")
      Add-RepLine "- Focus file updated."
    } else {
      Add-RepLine "- No changes applied."
    }
  } else {
    Add-RepLine "- Focus file missing. AutoFix skipped."
  }
}

Add-RepLine ""
Add-RepLine "## package.json scripts"
$scripts = Get-PackageScripts $pkgJson
if ($scripts.Count -eq 0) { Add-RepLine "- (none detected or JSON parse failed)" }
else { foreach ($k in ($scripts.Keys | Sort-Object)) { Add-RepLine ("- {0}: {1}" -f $k, $scripts[$k]) } }
...
function Run-NpmScriptIfExists([string]$scriptName, [string]$title) {
  if (-not $scripts.ContainsKey($scriptName)) {
    Add-RepLine ""
    Add-RepLine ("## {0}" -f $title)
    Add-RepLine ("- Skipped: script not found: {0}" -f $scriptName)
    return $null
  }
...
if ($scripts.ContainsKey("typecheck")) {
  Run-NpmScriptIfExists "typecheck" "Typecheck" | Out-Null
} else {
  ...
  if (Test-Path $tsc) {
    ...
    Add-RepLine ""
    Add-RepLine "## Typecheck"
    Add-RepLine "- Skipped: TypeScript not found in node_modules. Run with -Install."
  }
}
...
if ($DeepNpmAudit) {
  Add-RepLine ""
  Add-RepLine "## Deep npm audit"
  $o1 = Join-Path $toolsDir ("npm_ls_{0}.out.txt" -f $stamp)
  $e1 = Join-Path $toolsDir ("npm_ls_{0}.err.txt" -f $stamp)
  Invoke-LoggedCmd -Title "npm ls --depth=0" -WorkDir $RepoRoot -FilePath "npm" -ArgumentList @("ls","--depth=0") -StdOutPath $o1 -StdErrPath $e1 | Out-Null

  $o2 = Join-Path $toolsDir ("npm_prefix_{0}.out.txt" -f $stamp)
  $e2 = Join-Path $toolsDir ("npm_prefix_{0}.err.txt" -f $stamp)
  Invoke-LoggedCmd -Title "npm config get prefix" -WorkDir $RepoRoot -FilePath "npm" -ArgumentList @("config","get","prefix") -StdOutPath $o2 -StdErrPath $e2 | Out-Null
}

Focus-Checks $focusAbs

Add-RepLine ""
Add-RepLine "## Error Locations and Code Frames"
...
if ($locs.Count -eq 0) {
  Add-RepLine "- No file:line:col patterns detected in logs."
} else {
  $uniq = @{}
  foreach ($l in $locs) {
  ...
  foreach ($k in ($uniq.Keys | Sort-Object)) {
    $l = $uniq[$k]
    Add-RepLine ""
    Add-RepLine ("- Location: {0}:{1}:{2}" -f $l.File, $l.Line, $l.Col)
    Add-RepLine ("- Raw: {0}" -f $l.Raw)
...
if ($failed.Count -eq 0) { Add-RepLine "- All executed steps returned ExitCode 0." }
else { Add-RepLine ("- Failing steps: {0}" -f (($failed | ForEach-Object { $_.title + " (ExitCode " + $_.exit + ")" }) -join "; ")) }
