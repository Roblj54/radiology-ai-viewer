...
# Avoid alias collisions like r/R (Invoke-History)
...
Add-RepLine ("- Node: {0}" -f ($(if ($node.ok) { $node.text } else { "[MISSING] " + $node.text })))
Add-RepLine ("- npm: {0}"  -f ($(if ($npm.ok)  { $npm.text }  else { "[MISSING] " + $npm.text })))
Add-RepLine ("- git: {0}"  -f ($(if ($git.ok)  { $git.text }  else { "[MISSING] " + $git.text })))
