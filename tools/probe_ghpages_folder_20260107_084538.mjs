...
function summarizeText() {
  ...
  const slice = (txt.match(/Corte\s*\d+\s*\/\s*\d+/i) || txt.match(/Slice\s*\d+\s*\/\s*\d+/i) || [""])[0];
  const noSeries = /No series loaded/i.test(txt) || /No series loaded yet/i.test(txt) || /No hay series/i.test(txt);
...
(async () => {
  ...
  const errCount = logs.filter(x => x.type === "pageerror" || x.type === "requestfailed" || x.type === "console:error").length;
