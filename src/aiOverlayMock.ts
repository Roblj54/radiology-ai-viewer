export function setupAIMockOverlay(opts: { viewport: any; container: HTMLElement }) {
  ...
  const toWorld  = (x: number, y: number) => hasWorld ? viewport.canvasToWorld([x, y]) : [x, y, 0];
  const toCanvas = (p: any) => hasWorld ? viewport.worldToCanvas(p) : [p[0], p[1]];
  const dist3    = (a: any, b: any) => Math.hypot(a[0]-b[0], a[1]-b[1], (a[2]||0)-(b[2]||0));

  function resize() {
    const base: any =
      (viewport && typeof viewport.getCanvas === "function" && viewport.getCanvas()) ||
  ...
  function worldFromNorm(nx: number, ny: number) {
    resize();
  ...
  return {
    ...
    destroy() {
      ...
      window.removeEventListener("resize", redraw as any);
    }
  };
}
